...
loader at plugins/__init__.py:124–139 imports this module and calls
...
def setup(app: FastAPI, context: dict) -> None:
    ...
    async def upload_file(file: UploadFile = File(...)):
        # Wrap the whole handler in try/finally so file.close() always
        # runs — releasing the underlying spooled tempfile / fd that
        # FastAPI/Starlette allocates for the multipart body. The
        # framework will eventually close it on request teardown,
        # but explicit close releases resources promptly under
        # concurrent uploads.
        try:
    ...
    async def _do_upload(file: UploadFile):
        ...
        try:
            ...
            # Atomic replace FIRST: if it fails (disk full, locked
            # target on Windows, permission error), we want the
            # previous slot file to stay intact and playable. The
            # old-ext cleanup runs only after the new file is safely
            # in place — at worst the slot briefly contains both
            # current.mp4 and current.webm (renderer still picks the
            # right one via the persisted filename), never zero
            # files. If replace succeeds and the cleanup fails, the
            # next upload's same-pass cleanup or a future Clear
            # picks up the orphan.
            await run_in_threadpool(os.replace, str(tmp_path), str(out_path))

            for old in upload_dir.glob("current.*"):
                if old != out_path:
                    try:
                        await run_in_threadpool(old.unlink)
                    except OSError:
    ...
    async def delete_files():
        ...
        deleted = []
        leftover = []
        for path in upload_dir.glob("current.*"):
            try:
                await run_in_threadpool(path.unlink)
                deleted.append(path.name)
            except OSError as e:
                leftover.append({"name": path.name, "error": str(e)})
