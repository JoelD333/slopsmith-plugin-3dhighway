...
The whole plugin is **one file** — `screen.js`, wrapped in an IIFE, registered as `window.slopsmithViz_highway_3d` (a slopsmith#36 setRenderer factory). No build step, no imports beyond Three.js loaded lazily from a CDN.
...
3. **Three.js loader** — `loadThree()` (lazy CDN import, memoized)
...
- This plugin is a gitlink inside the parent slopsmith repo's `plugins/3dhighway/` directory. Resolve the full filesystem path from the active workspace rather than hardcoding it — paths and shell quirks (Windows vs WSL vs macOS, cygwin vs git-bash, etc.) are environment-specific.
